...
ssd_die_ftl_callback* simple_die_ftl::get_callback() const {
	return ftl_callback[0];
}
void simple_die_ftl::set_ftl_callback(ssd_die_ftl_callback* callback, int die_idx) {
	ftl_callback[die_idx] = callback;
}

void simple_die_ftl::reset_data_layout() {
	//lp_meta_table.resize(m_number_of_dice * USER_CAPACITY_IN_PAGE_PER_DIE);
	lp_meta_table = (lp_meta_t *) malloc(sizeof(lp_meta_t) * USER_CAPACITY_IN_PAGE_PER_DIE * m_number_of_dice);
	assert(lp_meta_table);

	p_meta_table = (p_meta_table_t **)malloc(m_number_of_dice*sizeof(void *));
	for (int i=0; i < m_number_of_dice; i++){
		p_meta_table[i] = create_p_meta_table(BLOCK_PER_DIE, PAGE_PER_BLOCK);
	...
	/* TO DO	random choose die_idx */
	vector<ppn_t> num_of_pp_has_been_filled(m_number_of_dice, 0) ;
	...
	for(lpn_t lpn = 0; lpn < m_number_of_dice * USER_CAPACITY_IN_PAGE_PER_DIE; lpn++) {
		while(1){
			die = rand() % m_number_of_dice;
			if(die_unused[die] != 0){
				die_unused[die]--;
		...
		ppn_t new_pp = write_lp_to_wr_ptr(p_meta_table[die], wr_ptr_for_gc[die], lpn);
	...
	for (int i=0; i < m_number_of_dice; i++){
		switch (slc_policy) {
			case NO_SLC_POLICY:
				wr_ptr_for_short_io_queue[i] = create_wr_ptr(p_meta_table[i], MLC_MODE);
				wr_ptr_for_long_io_queue[i] = create_wr_ptr(p_meta_table[i], MLC_MODE);
				break;
			case QUEUE_AWARE_SLC_POLICY:
				wr_ptr_for_short_io_queue[i] = create_wr_ptr(p_meta_table[i], MLC_MODE);
				wr_ptr_for_long_io_queue[i] = create_wr_ptr(p_meta_table[i], SLC_MODE);
				break;
			case ALWAYS_SLC_POLICY:
				wr_ptr_for_short_io_queue[i] = create_wr_ptr(p_meta_table[i], SLC_MODE);
				wr_ptr_for_long_io_queue[i] = create_wr_ptr(p_meta_table[i], SLC_MODE);
				break;
			default:
				assert(0);
		...
		assert(wr_ptr_for_short_io_queue[i]);
		assert(wr_ptr_for_long_io_queue[i]);
		assert(wr_ptr_for_gc[i]);
	}
...
bool simple_die_ftl::check_mapping_validity(lpn_t logical_page, ppn_t physical_page, int die_idx) const {

	assert(logical_page < USER_CAPACITY_IN_PAGE_PER_DIE * m_number_of_dice);

	ASSERT_IS_VALID_PPN(p_meta_table[die_idx], physical_page);

	ppn_t correct_physical_page = lp_meta_table[logical_page].ptr_to_physical_page;
	//assert(find_lpn_by_ppn(p_meta_table[die_idx], correct_physical_page) == logical_page);
	assert(find_lpn_by_ppn(p_meta_table[lp_meta_table[logical_page].die_idx], correct_physical_page) == logical_page);

	return correct_physical_page == physical_page;
}

msec_t simple_die_ftl::issue_page_read(lpn_t logical_page, bool generated_by_gc, int die_idx) {
...
msec_t simple_die_ftl::issue_page_write(lpn_t logical_page, bool generated_by_gc, int die_idx) {
	...
	ASSERT_IS_VALID_PPN(p_meta_table[die_idx], old_physical_page);

	if (generated_by_gc) {
		ASSERT_IS_VALID_TAIL_PB(p_meta_table[die_idx], find_pbn_by_ppn(p_meta_table[die_idx], old_physical_page));
	}
//cout << die_idx << " ";
//cout << die_idx << " lpn: " << logical_page << " " << p_meta_table[die_idx]->pp_meta_array[old_physical_page].ptr_to_logical_page << endl;
	//delete_pp_recorded_given_lpn(p_meta_table[die_idx], logical_page, old_physical_page);
	delete_pp_recorded_given_lpn(p_meta_table[lp_meta_table[logical_page].die_idx], logical_page, old_physical_page);

	/* update FTL */
	lp_meta_table[logical_page].ptr_to_physical_page = NULL_PPN;
	lp_meta_table[logical_page].die_idx = -1;
	/* ========================== */
	...
	if (generated_by_gc) {
		...
		/* wr_ptr_for_gc */
		new_pp = write_lp_to_wr_ptr(p_meta_table[die_idx], wr_ptr_for_gc[die_idx], logical_page);
		program_latency = _determine_write_latency(wr_ptr_for_gc[die_idx]);

	} else if (
			slc_policy == QUEUE_AWARE_SLC_POLICY
#ifdef DEADLINE_AWARE_SLC_POLICY
			&& !ftl_callback[die_idx]->can_all_pending_io_page_write_meet_deadline()
#ifdef GYC_PAPER_SLC_THROTTLING
			&& ((rand() % 100000) < (100000 * ftl_callback[die_idx]->get_slc_throttle_rate()))
#endif
		...
		new_pp = write_lp_to_wr_ptr(p_meta_table[die_idx], wr_ptr_for_long_io_queue[die_idx], logical_page);
		program_latency = _determine_write_latency(wr_ptr_for_long_io_queue[die_idx]);
		...
		new_pp = write_lp_to_wr_ptr(p_meta_table[die_idx], wr_ptr_for_short_io_queue[die_idx], logical_page);
		program_latency = _determine_write_latency(wr_ptr_for_short_io_queue[die_idx]);
...
msec_t simple_die_ftl::issue_block_erase(pbn_t physical_block, int die_idx) {
	erase_pb(p_meta_table[die_idx], physical_block);
...
*/
	...
	if (p_meta_table[die_idx]->num_of_free_pbs * 100 > p_meta_table[die_idx]->num_of_all_pbs * min_free_blocks_percentage) {
...
void simple_die_ftl::print_status(int die_idx) {

	FOR_EACH_PB(p_meta_table[die_idx], pbn, pb_meta) {
		cout << "for pb " << pbn << ": ";
		cout << "state: ";
		switch (pb_meta->state) {
			case PB_FREE:
				ASSERT_IS_VALID_FREE_PB(p_meta_table[die_idx], pbn);
				cout << "PB_FREE";
				break;
			case PB_HEAD:
				ASSERT_IS_VALID_HEAD_PB(p_meta_table[die_idx], pbn);
				cout << "PB_HEAD";
				break;
			case PB_BODY:
				ASSERT_IS_VALID_BODY_PB(p_meta_table[die_idx], pbn);
				cout << "PB_BODY";
				break;
			case PB_TAIL:
				ASSERT_IS_VALID_TAIL_PB(p_meta_table[die_idx], pbn);
				cout << "PB_TAIL";
				break;
			default:
				assert(0);
	...
	cout << "Total Free Block: " << p_meta_table[die_idx]->num_of_free_pbs << endl;
	cout << "wr_ptr_for_short_io_queue: " << wr_ptr_for_short_io_queue[die_idx]->ptr_to_head_pb << endl;
	cout << "wr_ptr_for_long_io_queue: " << wr_ptr_for_long_io_queue[die_idx]->ptr_to_head_pb << endl;
	cout << "wr_ptr_for_gc: " << wr_ptr_for_gc[die_idx]->ptr_to_head_pb << endl;
}

void simple_die_ftl::schedule_gc_if_needed(int die_idx) {

#ifdef GYC_PAPER_OVERLOAD_PROTECTION

	if (m_after_foreground_gc) {
		if (ftl_callback[die_idx]->get_number_of_pending_io_page_write() == 0) {
	...
	if (p_meta_table[die_idx]->num_of_free_pbs * 1000
			< p_meta_table[die_idx]->num_of_all_pbs * min_free_blocks_percentage) {
...
*/
	...
	if (p_meta_table[die_idx]->num_of_free_pbs <= 1) {
		if (ftl_callback[die_idx]->get_gc_priority() != FOREGROUND_GC) {
			ftl_callback[die_idx]->set_gc_priority(FOREGROUND_GC);
		}
	} else {
		if (ftl_callback[die_idx]->get_gc_priority() != BACKGROUND_GC) {
			ftl_callback[die_idx]->set_gc_priority(BACKGROUND_GC);
#ifdef GYC_PAPER_OVERLOAD_PROTECTION
			if (ftl_callback[die_idx]->get_number_of_pending_io_page_write() > 0 && !m_after_foreground_gc) {
	...
	if (ftl_callback[die_idx]->get_number_of_pending_gc_page_read() > 0) {
		return;
	}
	if (ftl_callback[die_idx]->get_number_of_pending_gc_page_write() > 0) {
		return;
	}
	if (ftl_callback[die_idx]->get_number_of_pending_gc_block_erase() > 0) {
		return;
	}
	...
	FOR_EACH_BODY_PB(p_meta_table[die_idx], pbn, pbn_meta) {
	...
	long double mlc_victim_utilization =
		static_cast<long double>(p_meta_table[die_idx]->pb_meta_array[mlc_victim_block].num_of_live_pages)
		/ PAGE_PER_BLOCK;
	bool case1=false;
	bool case2=false;
	bool case3=false;
	...
	if (slc_victim_block != NULL_PBN && mlc_victim_block != NULL_PBN) {
		...
		long double slc_victim_utilization = static_cast<long double>(p_meta_table[die_idx]->pb_meta_array[slc_victim_block].num_of_live_pages)
		...
		if (mlc_victim_utilization <= slc_victim_utilization / BIT_PER_MLC) {
			...
			//ftl_callback[die_idx]->record_big_d(mlc_victim_utilization * BIT_PER_MLC);
		} else if (slc_victim_utilization / BIT_PER_MLC < mlc_victim_utilization
				&& mlc_victim_utilization < slc_victim_utilization) {
			//ftl_callback[die_idx]->record_small_d(1.0 / BIT_PER_MLC);
			//ftl_callback[die_idx]->record_big_d(1.0 / BIT_PER_MLC);
			/*
			if (slc_victim_utilization < 1) {
				ftl_callback[die_idx]->record_small_d((1 - mlc_victim_utilization) / (1 - slc_victim_utilization));
			} else {
				ftl_callback[die_idx]->record_small_d((1 - mlc_victim_utilization) / (1.0 / PAGE_PER_BLOCK));
				ftl_callback[die_idx]->record_big_d(1.0);
			}
			*/

			//cout << name() << '\t' << slc_victim_utilization << '\t' << mlc_victim_utilization << endl;
			ftl_callback[die_idx]->record_big_d(slc_victim_utilization);
			ftl_callback[die_idx]->record_small_d(slc_victim_utilization * (slc_victim_utilization - mlc_victim_utilization));
			//ftl_callback[die_idx]->record_big_d(slc_victim_utilization);
			//ftl_callback[die_idx]->record_small_d(slc_victim_utilization);
			if (string(name()) == string("ssd.die_ftl_4")) {
	...
	//ftl_callback[die_idx]->record_small_d(0.0);
	//ftl_callback[die_idx]->record_big_d(0.0);

/*
	if (slc_victim_block != NULL_PBN) {
		ftl_callback[die_idx]->record_small_d(static_cast<long double>(p_meta_table->pb_meta_array[slc_victim_block].num_of_live_pages)
					/ (PAGE_PER_BLOCK / BIT_PER_MLC));
	}

	if (mlc_victim_block != NULL_PBN) {
		ftl_callback[die_idx]->record_big_d(static_cast<long double>(p_meta_table->pb_meta_array[mlc_victim_block].num_of_live_pages)
					/ PAGE_PER_BLOCK);
	}
*/
#endif
/*
	switch (p_meta_table->pb_meta_array[victim_old_physical_block].mode) {
		case PB_SLC:
			ftl_callback[die_idx]->record_small_d(1.0
...
*/
	...
	ppn_t min_num_of_live_pages = p_meta_table[die_idx]->num_of_pps_per_pb + 1;
	FOR_EACH_BODY_PB(p_meta_table[die_idx], pbn, pbn_meta) {
	...
	switch (p_meta_table[die_idx]->pb_meta_array[victim_old_physical_block].mode) {
		case PB_SLC:
			ftl_callback[die_idx]->schedule_gc_block_erase(victim_old_physical_block, true);
			break;
		case PB_MLC:
			ftl_callback[die_idx]->schedule_gc_block_erase(victim_old_physical_block, false);
			break;
		default:
			assert(0);
			break;
...
simple_die_ftl::simple_die_ftl(sc_module_name module_name, int number_of_dice) : sc_module(module_name) {
	...
	wr_ptr_for_long_io_queue.resize(m_number_of_dice, NULL);
	wr_ptr_for_gc.resize(m_number_of_dice, NULL);
...
bool simple_die_ftl::should_enable_slc(int die_idx) const {
	return true;
	...
	if (p_meta_table[die_idx]->num_of_free_pbs * 200 /* i.e. 50% free space usage */
			> p_meta_table->num_of_all_pbs * min_free_blocks_percentage) {
		return true;
	}

	long double throttle_level
		= static_cast<long double>(p_meta_table[die_idx]->num_of_free_pbs) * 200
		/ (p_meta_table[die_idx]->num_of_all_pbs * min_free_blocks_percentage);
