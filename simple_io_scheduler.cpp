...
void simple_io_scheduler::trace_replayer() {
	...
	while ( new_tran != NULL ||  m_trace_fs >> req_time >> req_dev >> req_addr >> req_size >> req_op) {
		...
		if(get_last_time_for_save() > sc_time(0, SC_MS) && sc_time(req_time, SC_MS) > get_last_time_for_save()){
			//save from here
			break;
		}
	...
	upload_ch_port("upload_ch_port")
{
	...
	if(last == 0){ //load from which line //sc_time(0, SC_MS)
		double tmp_time;
		ifstream trace_last_ifs;
		trace_last_ifs.open((trace_filename + ".last").c_str());
		trace_last_ifs >> tmp_time;
		trace_last_ifs.close();
		cout << tmp_time <<" for init"<< endl;
		
		set_last_time_for_save(sc_time(0, SC_MS));
		set_last_time_for_load(sc_time(tmp_time, SC_MS));
	}
	else if(last > 0) { //save how many request
		set_last_time_for_save(sc_time(last, SC_MS));
		set_last_time_for_load(sc_time(0, SC_MS));
	}
	else{
		set_last_time_for_save(sc_time(0, SC_MS));
		set_last_time_for_load(sc_time(0, SC_MS));
